...
public class ThreeMainActivity extends Activity {
	...
	public class Connnet extends Thread {
		...
		public void run() {
			try {
				System.out.println("自动连接线程");
				Thread.sleep(10000);
				SendData.dataInit(Changliang.SHAKE);
				System.out.println("连接线程结束");
	...
	public void onclick(View v) {
		...
		case R.id.cancalshake:// 震动
			if(vibrator_temp){//取消震动
				vibrator.cancel();
				SendData.dataInit(Changliang.CANCALSHAKE);
				cancalshake.setText("震动");
				vibrator_temp = false;
				
			}else{//震动
				SendData.dataInit(Changliang.SHAKE);
				cancalshake.setText("取消震动");
				vibrator_temp = true;
				
			}
			break;
		
		case R.id.lock:// 锁定与解锁
			if (lock_temp) {
	...
	/**
	 * 
	 * sql_step:(对获得的数据进行储存) TODO(这里描述这个方法适用条件 – 可选) TODO(这里描述这个方法的执行流程 – 可选)
	 * TODO(这里描述这个方法的使用方法 – 可选) TODO(这里描述这个方法的注意事项 – 可选)
	 *
	 * @param @param
	 *            v 设定文件
